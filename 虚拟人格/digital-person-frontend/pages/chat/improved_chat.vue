<template>
  <view class="chat-container">
    ...
    <view class="input-area">
      <!-- 聊天模式切换 -->
      <view class="chat-mode-selector" v-if="enableFunctionCalling && enableStreaming">
        <text class="mode-label">模式:</text>
        <picker
          :range="['流式聊天', '工具助手']"
          :value="chatMode === 'streaming' ? 0 : 1"
          @change="onModeChange"
          class="mode-picker"
        >
          <view class="picker-display">
            <text>{{ chatMode === 'streaming' ? '流式聊天' : '工具助手' }}</text>
...
export default {
  ...
  data() {
    return {
      ...
      // Chat mode settings
      chatMode: 'streaming', // 'streaming' or 'function'
      enableFunctionCalling: config.features.enableFunctionCalling,
      enableStreaming: config.features.enableStreaming
    }
  },
  created() {
  ...
  methods: {
    ...
    async loadAvailableTools() {
      try {
        const response = await uni.request({
          url: `${config.api.baseUrl}/api/tools`,
          method: 'GET'
        })
    ...
    async sendMessage() {
      ...
      try {
        if (this.chatMode === 'function' && this.enableFunctionCalling) {
          // 显式使用Function Calling模式
    ...
    async sendFunctionCallMessage(userMessage) {
      try {
        const response = await uni.request({
          url: `${config.api.baseUrl}/api/chat/function`,
          method: 'POST',
          header: {
        ...
        console.error('Function Calling失败，降级到流式聊天:', error)
        // 降级到流式聊天
        await this.sendStreamingMessage(userMessage)
      }
    },
    ...
    async sendStreamingMessage(userMessage) {
      return new Promise((resolve, reject) => {
        ...
        // 创建SSE连接
        const eventSource = new EventSource(`${config.api.baseUrl}/api/chat/stream?message=${encodeURIComponent(userMessage)}`)
        ...
        eventSource.onerror = (error) => {
          ...
          // 降级到普通API
          this.fallbackToNormalChat(userMessage, messageIndex)
            .then(resolve)
            .catch(reject)
        }
      })
    ...
    async fallbackToNormalChat(userMessage, messageIndex) {
      try {
        console.log('流式API失败，降级到普通API')
        const response = await uni.request({
          url: `${config.api.baseUrl}/api/chat`,
          method: 'POST',
          header: {
            'Content-Type': 'application/json'
          },
        ...
        const result = response[1].data
        if (result.success) {
          this.messages[messageIndex].content = result.response
          this.messages[messageIndex].isStreaming = false
          this.messages[messageIndex].isMarkdown = this.containsMarkdown(result.response)
        } else {
          this.messages[messageIndex].content = '抱歉，服务器出错了，请稍后再试。'
          this.messages[messageIndex].isStreaming = false
        }
      } catch (error) {
        console.error('普通API也失败了:', error)
        this.messages[messageIndex].content = '抱歉，服务器出错了，请稍后再试。'
        this.messages[messageIndex].isStreaming = false
      }
    },
    ...
    onModeChange(event) {
      ...
      this.chatMode = index === 0 ? 'streaming' : 'function'
    },
